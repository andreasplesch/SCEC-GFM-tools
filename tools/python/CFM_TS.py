class CFM_TS(object):
    # Slighlty modified from python_geoprobe
    ...
    def _read_tsurf(self, filename):
        # global infile, line, Color, Vertices, Triangles
        with open(filename, 'r') as infile:
            ...
            if line.startswith('HEADER'):
                ...
                # self.name = self.self.header.get('name', filename)
                try:
                    # Color = [float(item) for item in self.header['color'].split()]
                    self.color = [val for key, val in self.header.items() if 'color' in key]
            ...
            if line.startswith('GOCAD_ORIGINAL_COORDINATE_SYSTEM'):
                self.GOCAD_coordinate_system = {}
                line = next(infile).strip()
                while 'END' not in line:
                    key, value = line.split(' ', 1)
                    self.GOCAD_coordinate_system[key.lstrip()] = value
                    line = next(infile).strip()
    ...
    def _init_from_xyz(self, x, y, z, triangles):
        ...
        self.triangles = list(zip(triangles[:, 0], triangles[:, 1], triangles[:, 2]))
